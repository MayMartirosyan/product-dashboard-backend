...
export class ProductController {
  ...
  static async getMyProducts(req: Request, res: Response) {
    try {
      ...
      const productRepository = AppDataSource.getRepository(Product);
      const products = await productRepository.find({ where: { user: { id: userId } }, relations: ['user'] });
      return res.status(200).json(products);
    } catch (error: any) {
      console.error("GetMyProducts error:", error);
      return res.status(500).json({ message: error.message || 'Failed to fetch user products' });
    }
  }
  ...
  static async getProductById(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const productRepository = AppDataSource.getRepository(Product);
      const product = await productRepository.findOne({ where: { id }, relations: ['user'] });
      if (!product) {
  ...
  static async addProduct(req: Request, res: Response) {
    try {
      ...
      const parsedDiscountedPrice = discountedPrice ? parseFloat(discountedPrice) : undefined;

      if (parsedDiscountedPrice !== undefined && parsedDiscountedPrice >= parsedPrice) {
      ...
      return res.status(400).json({ message: error.message || 'Failed to add product' });
    }
  }
  ...
  static async updateProduct(req: Request, res: Response) {
    try {
      ...
      const productRepository = AppDataSource.getRepository(Product);
      const product = await productRepository.findOne({ where: { id, user: { id: userId } } });
      if (!product) {
      ...
      const parsedDiscountedPrice = discountedPrice ? parseFloat(discountedPrice) : product.discountedPrice;
